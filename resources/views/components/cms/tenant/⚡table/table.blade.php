<div>
        ...
        <flux:table.rows>
            @forelse($data as $d)
                <flux:table.row>
                        ...
                        <flux:dropdown>
                            ...
                            <flux:menu>
                                @can('update' . $this->modelInstance)
                                    <flux:menu.item variant="default" icon="pencil"
                                        @click="$flux.modal('defaultModal').show(); $wire.dispatch('set-action', { id: '{{ $d->id }}' });">
                                        Update
                                    </flux:menu.item>
                                    <flux:menu.item variant="default" icon="identification"
                                        @click="$wire.dispatch('show-profile', { id: '{{ $d->id }}' });">
                                        Profile
                                    </flux:menu.item>
                                    @if (auth()->user()->isSuperAdmin())
                                        <flux:menu.item variant="default" icon="tv"
                                ...
                                @can('delete' . $this->modelInstance)
                                    <flux:menu.item variant="danger" icon="trash"
                                        @click="$wire.dispatch('confirm', { function: 'delete', id: '{{ $d->id }}' })">
                                        Delete
                                    </flux:menu.item>
                                @endcan
                            </flux:menu>
                    ...
                    <flux:table.cell>
                        <flux:badge color="{{ $d->type->color() }}" size="sm">{{ $d->type->label() }}</flux:badge>
                    </flux:table.cell>
                    <flux:table.cell>{{ $d->branch }}</flux:table.cell>
                    <flux:table.cell>{{ $d->email }}</flux:table.cell>
                    <flux:table.cell>
                        <flux:badge color="{{ $d->status->color() }}" size="sm">{{ $d->status->label() }}</flux:badge>
                    </flux:table.cell>
    ...
    <livewire:cms.tenant.create-update lazy />
    <livewire:cms.tenant.profile lazy />
</div>
