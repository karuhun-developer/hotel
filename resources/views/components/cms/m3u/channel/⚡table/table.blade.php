<div>
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-0 mt-5 mb-4">
        <div class="w-full md:w-auto flex flex-wrap items-center gap-4">
            <x-ui.filter.select :data="$this->sources" model="sourceFilter" label="Filter Source" placeholder="-- All Sources --" changeEvent="" />
        </div>
    </div>
        ...
        <flux:table.rows>
            @forelse($data as $d)
                <flux:table.row>
                                ...
                                @can('update' . $this->modelInstance)
                                    <flux:menu.item
                                        ...
                                        @click="$wire.dispatch('confirm', {
                                            ...
                                            id: '{{ $d->id }}',
                                            title: 'Toggle Status',
                                            message: 'Toggle status for this channel?',
                                        })">
                                        Toggle Status
                                    </flux:menu.item>
                                @endcan
                    ...
                    <flux:table.cell>{{ $d->alias }}</flux:table.cell>
                    <flux:table.cell>
    ...
    <livewire:cms.m3u.channel.create-update lazy />
</div>
