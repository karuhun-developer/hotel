            ...
            <x-ui.table.tbody>
                @forelse($data as $d)
                    <tr>
                        ...
                        <x-ui.table.td>
                            {{ $d->food_category_name }}
                        </x-ui.table.td>
                        <x-ui.table.td>
                            {{ $d->name }}
                        </x-ui.table.td>
                        <x-ui.table.td>
                            {{ numberToCurrency($d->price) }}
                        </x-ui.table.td>
                        <x-ui.table.td>
                            {!! $d->description !!}
                        </x-ui.table.td>
        ...
        <form class="space-y-6" wire:submit.prevent="submit">
            ...
            @if (auth()->user()->isSuperAdmin())
                <flux:field>
                    ...
                    <flux:select wire:model.live="tenant_id" placeholder="Select tenant ...." wire:change="getFoodCategories">
            ...
            <flux:field>
                <flux:label>Food Category</flux:label>
                <flux:text>Select the food category this food belongs to.</flux:text>
                <flux:select wire:model.live="food_category_id" id="food_category_id"  placeholder="Select Food Category ....">
            ...
            <flux:field>
                ...
                <flux:text>Food price.</flux:text>
                <flux:input wire:model="price" type="text" x-mask:dynamic="$money($input, ',')" />
                <flux:error name="price" />
            </flux:field>

            <flux:field>
                <flux:label>Description</flux:label>
                <flux:text>Food description.</flux:text>
